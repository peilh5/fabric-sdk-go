...
func (c *certPool) Get() (*x509GM.CertPool, error) {
...
func (c *certPool) Add(certs ...*x509GM.Certificate) {
	if len(certs) == 0 {
		return
	}
...
func (c *certPool) filterCerts(certs ...*x509GM.Certificate) []*x509GM.Certificate {
	c.lock.RLock()
	defer c.lock.RUnlock()

	var filtered []*x509GM.Certificate
...
func removeDuplicates(certs ...*x509GM.Certificate) []*x509GM.Certificate {
	encountered := map[*x509GM.Certificate]bool{}
	var result []*x509GM.Certificate
...
func loadSystemCertPool(useSystemCertPool bool) (*x509GM.CertPool, error) {
	if !useSystemCertPool {
		return x509GM.NewCertPool(), nil
	}
	systemCertPool, err := x509GM.SystemCertPool()
	if err != nil {
		return nil, err
	}
